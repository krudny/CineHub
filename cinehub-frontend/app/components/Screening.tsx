...
export default function Screening({ id }: { id: number }) {
  ...
  const { data } = useSWR<Screening[]>(`http://localhost:8080/screenings?movieId=${id}`, fetcher);
  const [selectedDate, setSelectedDate] = useState<string>("");
  const [selectedScreening, setSelectedScreening] = useState<Screening | null>(null);
  const router = useRouter();

  const uniqueDates = data
      ? Array.from(new Set(data.map((screening) => screening.startDate.split("T")[0]))).sort()
      : [];
            ...
            <>
              ...
              <div className="flex items-center gap-x-4 mt-2">
                ...
                <div className="flex items-center gap-x-4">
                  {data
                      ?.filter((screening) => screening.startDate.startsWith(selectedDate))
                      .map((screening) => (
                          <p
              ...
              >
                <p className="text-zinc-900 font-bold text-md lg:text-lg">Choose seat</p>
              </button>
            </>
        ) : (
            <p className="text-lg text-neutral-500 mt-10">No screenings available.</p>
        )}
      </div>
  );
}
